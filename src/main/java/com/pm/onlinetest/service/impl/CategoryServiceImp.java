...
public class CategoryServiceImp implements CategoryService {
	...
	//  function to check Category already exists or not
	@Override
	public boolean checkCategory(Category category){
		List<Category> catagoryList = categoryRepository.findAllEnabled();
		for(Category cata : catagoryList){
			
			if(category.getName().equalsIgnoreCase(cata.getName())){
				return false;
			}
		}
		return true;
